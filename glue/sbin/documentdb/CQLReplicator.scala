...
case class DocdbConfig(pwd: String, usr: String, clusterDnsName: String, clusterPort: Int, sslEnabled: Boolean, maxAttempts: Int, useXXHash64Key: Boolean, connectionTimeout: Int, soTimeout: Int, xxHash64Seed: Long)
...
object GlueApp {
  def main(sysArgs: Array[String]) {
    ...
    def readDocumentDBConfigFile(s3Client: com.amazonaws.services.s3.AmazonS3, bucket: String, key: String): DocdbConfig = {
      val s3Object = s3Client.getObject(bucket, key)
      val src = Source.fromInputStream(s3Object.getObjectContent())
      val json = src.getLines.mkString
      src.close()
      ...
      implicit val formats = DefaultFormats
      parse(json).extract[DocdbConfig]
    }
    ...
    val docdbConfig = readDocumentDBConfigFile(s3client, bcktName, "artifacts/DocumentDb.conf")
    ...
    lazy val computeHash = (key: String, factory: net.jpountz.xxhash.StreamingXXHash64) => {
      docdbConfig.useXXHash64Key match {
        case true => {
    ...
    def persistToTarget(df: DataFrame, columns: scala.collection.immutable.Map[String, String],
      ...
      df.rdd.foreachPartition(
        partition => {
          ...
          lazy val xxHash64Seed = xxHashFactory.newStreamingHash64(docdbConfig.xxHash64Seed)
          lazy val docdbCluster = getDocdbConnection(docdbConfig, s"CQLReplicator$currentTile")
          partition.foreach(
            row => {
