...
case class CompressionConfig(enabled: Boolean = false, compression: String = "lz4", compressionLevel: Int = 3, compressNonPrimaryColumns: List[String] = List(""), compressAllNonPrimaryKeysColumns: Boolean = false, targetAttributeName: String = "") {
  ...
  def main(sysArgs: Array[String]): Unit = {
    ...
    def downloadDictionaryFromS3(s3Client: AmazonS3, bucket: String, key: String): Array[Byte] = {
      Try {
        ...
        case Failure(e) => throw CompressionException(s"Dictionary not found at s3://$bucket/$key: ${e.getMessage}")
        case Success(bytes) =>
      ...
      if (jsonMapping4s.dynamodb.compressionConfig.enabled) {
        ...
        val updatedJson = excludedJson.isEmpty match {
          case false => {
            ...
            val compressedPayload: Array[Byte] = jsonMapping4s.dynamodb.compressionConfig.compression match {
    ...
    def keysDiscoveryProcess(): Unit = {
      ...
      val compressionCfg = jsonMapping4s.dynamodb.compressionConfig
      if (compressionCfg.compression == "zstd-dict-based") {
        ...
        if (cleanUpRequested || !s3client.doesObjectExist(bcktName, dictKey)) {
          ...
          val sampleContent = try {
            ...
            case e: Exception =>
              throw CompressionException(s"Sample file not found at s3://$bcktName/$sampleKey: ${e.getMessage}")
          }

          implicit val formats: DefaultFormats.type = DefaultFormats
          val samples = parse(sampleContent).extract[List[String]]
          if (samples.isEmpty) {
            throw CompressionException("Sample file contains an empty list; cannot train dictionary")
          }
          ...
          val dictBytes = try {
            ...
            case e: com.github.luben.zstd.ZstdException =>
              throw CompressionException(s"Dictionary training failed: ${e.getMessage}. Ensure the sample file contains enough entries (at least 10 representative samples)")
          }
          if (dictBytes == null || dictBytes.isEmpty) {
            throw CompressionException("Dictionary training produced empty result")
          }
          ...
          try {
            ...
            if (!java.util.Arrays.equals(input, decompressed)) {
              throw CompressionException("Dictionary verification failed: round-trip compress/decompress mismatch")
            }
          } finally {
            dictCompress.close()
      ...
      _ => {
        processType match {
          ...
          case "replication" => {
            ...
            if (jsonMapping4s.dynamodb.compressionConfig.enabled && jsonMapping4s.dynamodb.compressionConfig.compression == "zstd-dict-based") {
              val dictKey = s"artifacts/${srcKeyspaceName}_${srcTableName}_dictionary.bin"
              ...
              zstdLevel = jsonMapping4s.dynamodb.compressionConfig.compressionLevel
            }
            if (replayLog) {
