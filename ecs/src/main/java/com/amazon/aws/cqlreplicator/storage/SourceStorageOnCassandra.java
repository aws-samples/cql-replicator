...
public class SourceStorageOnCassandra {
    ...
    public SourceStorageOnCassandra(Properties config) {
        ...
        metaData =
                getColumns(config.getProperty("SOURCE_KEYSPACE"), config.getProperty("SOURCE_TABLE"));
        var partitionKeyNames =
                metaData.get("partition_key").keySet().toArray(new String[0]);
        config.setProperty("SOURCE_CQL_QUERY", Utils.getSourceQuery(config, partitionKeyNames));
