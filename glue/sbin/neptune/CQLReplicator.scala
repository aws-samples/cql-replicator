...
class NeptuneConnectionException(s: String) extends RuntimeException {
  println(s)
}
...
class NeptuneException(s: String) extends RuntimeException {
  println(s)
}
...
case class Transformation(enabled: Boolean = false, fieldsMapping: List[FieldsTransformation] = List(FieldsTransformation()), filterExpression: String = "")
case class GraphProperties(properties: List[Map[String, String]])
case class GraphVertex(label: String, graphProperties: GraphProperties = GraphProperties(List(Map())))
case class GraphEdge(label: String, graphProperties: GraphProperties = GraphProperties(List(Map())))
case class GraphEntity(vertexOut: GraphVertex, vertexIn: GraphVertex, edge: GraphEdge)
case class GraphSchema(graphEntity: List[GraphEntity])
case class NeptuneConfig(enableUpsert: Boolean = false, enableDeletes: Boolean = false, maxFileSizeMb: Int = 32, transformation: Transformation = Transformation(), graphSchema: GraphSchema = GraphSchema())
case class JsonMapping(replication: Replication, neptune: NeptuneConfig)
sealed trait Stats
...
case class NeptuneConnectionConfig(
                     neptuneWriteEndpoint: String,
                     neptunePort: Int = 8182,
                     neptuneRegion: String,
                     neptuneEnableSsl: Boolean = true,
                     neptuneEnableIamAuth: Boolean = true)

case class FlushingSet[T](maxSize: Int, flushingClient: com.amazonaws.services.s3.AmazonS3, bucketName: String, key: String, ss: org.apache.spark.sql.SparkSession) {
  ...
  def flush(): DataFrame = this.synchronized {
    if (set.nonEmpty) {
      val jsonData = set.mkString("\n")
      set = ParSet.empty
      size = 0
      ss.read.option("multiLine", true).json(jsonData)
    }
  }
...
object GlueApp {
  def main(sysArgs: Array[String]): Unit = {
    ...
    def readNetpuneConnectionConfigFile(s3Client: com.amazonaws.services.s3.AmazonS3, bucket: String, key: String): NeptuneConnectionConfig = {
      val s3Object = s3Client.getObject(bucket, key)
      val src = Source.fromInputStream(s3Object.getObjectContent)
      val json = src.getLines.mkString
      src.close()
      ...
      implicit val formats: DefaultFormats.type = DefaultFormats
      parse(json).extract[NeptuneConfig]
    }
    ...
    def getNeptuneConnectionFactory(connectionConf: NeptuneConnectionConfig): GremlinCluster = {
      Try {
        NeptuneGremlinClusterBuilder.build()
            .enableSsl(connectionConf.neptuneEnableSsl)
            .enableIamAuth(connectionConf.neptuneEnableIamAuth)
            .addContactPoints(connectionConf.neptuneWriteEndpoint)
            .serviceRegion(connectionConf.neptuneRegion)
            .port(connectionConf.neptunePort)
            .create();
      } match {
        case Failure(e) => throw new NeptuneException(s"Unable to connect to Amazon Neptune service: $e")
        case Success(neptuneCluster) => neptuneCluster
      }
    }
    ...
    val neptuneConfig = readNetpuneConnectionConfigFile(s3client, bcktName, "artifacts/NeptuneConnector.conf")
    ...
    def retrieveDataFromSource(df: DataFrame, columns: scala.collection.immutable.Map[String, String],
                        columnsPos: scala.collection.immutable.SortedSet[(String, Int)], tile: Int, op: String): DataFrame = {
      df.rdd.foreachPartition(
        partition => {
          ...
          val gremlinCluster: GremlinCluster = getNeptuneConnectionFactory(neptuneConfig)
    ...
    def jsonToNeptune(df: DataFrame, op: String, tile: Int): Long = {
      val cnt = if (df.isEmpty) {
        0
      } else {
        retrieveDataFromSource(shuffleDfV2(df.drop("ts", "group")), columns, columnsPos, tile, op)
        if (op != "delete") {
          ...
          val transformedDf = if (jsonMapping4s.neptune.enabled) {
            dfFromJson.applyMapping(transformation, caseSensitive = false, transformationContext = s"transformation for $srcTableName")
            if (jsonMapping4s.neptune.transformation.filterExpression.nonEmpty) {
              val filterExpr = jsonMapping4s.neptune.transformation.filterExpression
              DynamicFrame.fromDF(dfFromJson.toDF().filter(filterExpr), glueContext, "dfFromJson")
            }
          } else {
            dfFromJson
          }
